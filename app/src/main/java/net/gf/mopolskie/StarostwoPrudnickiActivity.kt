...
class StarostwoPrudnickiActivity : ComponentActivity() {
    private lateinit var viewModel: Starostwo9ViewModel

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_prudnicki_starostwo)
        viewModel = ViewModelProvider(this)[Starostwo9ViewModel::class.java]
        ...
        lifecycleScope.launch {
            ...
            if (success) {
                ...
                Toast.makeText(this@StarostwoPrudnickiActivity, "Błąd podczas pobierania danych.", Toast.LENGTH_SHORT).show()
            }
        }
    }

    private fun displayWords(words: List<Starostwo9Location>) {
...
class Starostwo9ViewModel : ViewModel() {
    val words = mutableListOf<Starostwo9Location>()

    suspend fun fetchWords(): Boolean {
        return withContext(Dispatchers.IO) {
            try {
                val response = ApiStarostwo9Service.create().getStarostwo1()
                if (response.isSuccessful) {
                    val map = response.body()?.Prudnicki ?: return@withContext false
                    for ((_, obj) in map) {
                        val json = ApiStarostwo9Service.gson.toJsonTree(obj).asJsonObject
                        val location = ApiStarostwo9Service.gson.fromJson(json, Starostwo9Location::class.java)
                        words.add(location)
                    }
                    true
                } else {
...
interface ApiStarostwo9Service {
    @GET("/__api/mopolskie/starostwo/10")
    suspend fun getStarostwo1(): Response<Starostwo9Response>

    companion object {
        private const val BASE_URL = "https://api.goflux.pl"
        val gson = com.google.gson.GsonBuilder().create()

        fun create(): ApiStarostwo9Service {
            return Retrofit.Builder()
                .baseUrl(BASE_URL)
                .addConverterFactory(GsonConverterFactory.create(gson))
                .build()
                .create(ApiStarostwo9Service::class.java)
        }
    }
}

data class Starostwo9Response(
    val Prudnicki: Map<String, Any>
)

data class Starostwo9Location(
    val name: String,
    val branch: String,
    val city: String,
    val address: String,
