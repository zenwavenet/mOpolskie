...
class WywozSmieciKKBlachowniaActivity : ComponentActivity() {
    private lateinit var viewModel: Waste2ViewModel

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.waste_kedzierzyn_kozle_blachownia)
        viewModel = ViewModelProvider(this)[Waste2ViewModel::class.java]
        ...
        lifecycleScope.launch {
            Toast.makeText(this@WywozSmieciKKBlachowniaActivity, "Trwa parsowanie danych...", Toast.LENGTH_SHORT).show()
            val success = viewModel.fetchWaste()
            if (success) {
                showWaste(viewModel.items)
            } else {
                Toast.makeText(this@WywozSmieciKKBlachowniaActivity, "Nie udało się pobrać danych", Toast.LENGTH_SHORT).show()
            }
        }
    }
...
interface WasteApi2Service {
    @GET("/__api/mopolskie/wywoz/448")
    suspend fun getWasteSchedule(): Response<WasteResponse>

    companion object {
        private const val BASE_URL = "http://192.168.1.103:3000"

        fun create(): WasteApi2Service {
            return Retrofit.Builder()
                .baseUrl(BASE_URL)
                .addConverterFactory(GsonConverterFactory.create())
                .build()
                .create(WasteApi2Service::class.java)
        }
    }
}
