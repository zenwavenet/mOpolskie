...
class WywozSmieciKKFirmyActivity : ComponentActivity() {
    private lateinit var viewModel: Waste15ViewModel

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.waste_kedzierzyn_kozle_firmy)
        viewModel = ViewModelProvider(this)[Waste15ViewModel::class.java]
        ...
        lifecycleScope.launch {
            Toast.makeText(this@WywozSmieciKKFirmyActivity, "Trwa parsowanie danych...", Toast.LENGTH_SHORT).show()
            val success = viewModel.fetchWaste()
            if (success) {
                showWaste(viewModel.items)
            } else {
                Toast.makeText(this@WywozSmieciKKFirmyActivity, "Nie udało się pobrać danych", Toast.LENGTH_SHORT).show()
            }
        }
    }
...
interface WasteApi15Service {
    @GET("/__api/mopolskie/wywoz/499")
    suspend fun getWasteSchedule(): Response<WasteResponse>

    companion object {
        private const val BASE_URL = "https://api.goflux.pl"

        fun create(): WasteApi15Service {
            return Retrofit.Builder()
                .baseUrl(BASE_URL)
                .addConverterFactory(GsonConverterFactory.create())
                .build()
                .create(WasteApi15Service::class.java)
        }
    }
}
