...
class StarostwoKedzierzynskoKozielskieActivity : ComponentActivity() {
    private lateinit var viewModel: Starostwo2ViewModel

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_kedzierzynsko_kozielski_starostwo)
        viewModel = ViewModelProvider(this)[Starostwo2ViewModel::class.java]
    ...
    private fun displayWords(words: List<Starostwo2Location>) {
...
interface ApiStarostwo2Service {
    @GET("/__api/mopolskie/starostwo/3")
    suspend fun getStarostwo1(): Response<Starostwo2Response>

    companion object {
        private const val BASE_URL = "http://192.168.1.103:3000"
        val gson = com.google.gson.GsonBuilder().create()

        fun create(): ApiStarostwo2Service {
            return Retrofit.Builder()
                .baseUrl(BASE_URL)
                .addConverterFactory(GsonConverterFactory.create(gson))
                .build()
                .create(ApiStarostwo2Service::class.java)
        }
    }
}

data class Starostwo2Response(
    val KedzierzynskoKozielski: Map<String, Any>
)

data class Starostwo2Location(
    val name: String,
    val branch: String,
    val city: String,
    val address: String,
