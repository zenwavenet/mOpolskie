...
class WywozSmieciDobrodzienActivity : ComponentActivity() {
    ...
    private fun showRegionMenu() {
        setContentView(R.layout.activity_waste_dobrodzien)
        setupStatusBar()
    ...
    private fun setupRegionButtons() {
        setupRegionButton(R.id.bakij, "baki_j")
        setupRegionButton(R.id.bakiw, "baki_w")
        setupRegionButton(R.id.bakif, "baki_f")
        setupRegionButton(R.id.blachowj, "blachow_j")
        setupRegionButton(R.id.blachoww, "blachow_w")
        setupRegionButton(R.id.blachowf, "blachow_f")
        setupRegionButton(R.id.dobrodzienj, "dobrodzien_j")
        setupRegionButton(R.id.dobrodzienw, "dobrodzien_w")
        setupRegionButton(R.id.dobrodzienf, "dobrodzien_f")
        setupRegionButton(R.id.warlowj, "warlow_j")
        setupRegionButton(R.id.warloww, "warlow_w")
        setupRegionButton(R.id.warlowf, "warlow_f")
    }
    ...
    private fun loadWasteData(endpointId: Int) {
        lifecycleScope.launch {
            ...
            if (success) {
                ...
                Toast.makeText(this@WywozSmieciDobrodzienActivity, "Nie udało się pobrać danych.", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun getRegionConfig(region: String): RegionConfig {
        return when (region) {
            "baki_j" -> RegionConfig(R.layout.waste_dobrodzien_baki_j, WasteEndpoints.DOBRODZIEN_BAKI_J)
            "baki_w" -> RegionConfig(R.layout.waste_dobrodzien_baki_w, WasteEndpoints.DOBRODZIEN_BAKI_W)
            "baki_f" -> RegionConfig(R.layout.waste_dobrodzien_baki_f, WasteEndpoints.DOBRODZIEN_BAKI_F)
            "blachow_j" -> RegionConfig(R.layout.waste_dobrodzien_blachow_j, WasteEndpoints.DOBRODZIEN_BLACHOW_J)
            "blachow_w" -> RegionConfig(R.layout.waste_dobrodzien_blachow_w, WasteEndpoints.DOBRODZIEN_BLACHOW_W)
            "blachow_f" -> RegionConfig(R.layout.waste_dobrodzien_blachow_f, WasteEndpoints.DOBRODZIEN_BLACHOW_F)
            "dobrodzien_j" -> RegionConfig(R.layout.waste_dobrodzien_j, WasteEndpoints.DOBRODZIEN_J)
            "dobrodzien_w" -> RegionConfig(R.layout.waste_dobrodzien_w, WasteEndpoints.DOBRODZIEN_W)
            "dobrodzien_f" -> RegionConfig(R.layout.waste_dobrodzien_f, WasteEndpoints.DOBRODZIEN_F)
            "warlow_j" -> RegionConfig(R.layout.waste_dobrodzien_warlow_j, WasteEndpoints.DOBRODZIEN_WARLOW_J)
            "warlow_w" -> RegionConfig(R.layout.waste_dobrodzien_warlow_w, WasteEndpoints.DOBRODZIEN_WARLOW_W)
            "warlow_f" -> RegionConfig(R.layout.waste_dobrodzien_warlow_f, WasteEndpoints.DOBRODZIEN_WARLOW_F)
            else -> RegionConfig(R.layout.waste_dobrodzien_j, WasteEndpoints.DOBRODZIEN_J)
        }
    }
    ...
    private fun setupFavoriteButton(region: String) {
        ...
        if (favoriteButton != null && favoriteIcon != null && favoriteText != null) {
            ...
            println("WywozSmieciDobrodzienActivity: Nie znaleziono elementów UI dla favoriteButton")
        }
    }
