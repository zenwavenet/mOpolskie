...
class WywozSmieciKKSrodmiescieActivity : ComponentActivity() {
    private lateinit var viewModel: Waste8ViewModel

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.waste_kedzierzyn_kozle_srodmiescie)
        viewModel = ViewModelProvider(this)[Waste8ViewModel::class.java]
        ...
        lifecycleScope.launch {
            Toast.makeText(this@WywozSmieciKKSrodmiescieActivity, "Trwa parsowanie danych...", Toast.LENGTH_SHORT).show()
            val success = viewModel.fetchWaste()
            if (success) {
                showWaste(viewModel.items)
            } else {
                Toast.makeText(this@WywozSmieciKKSrodmiescieActivity, "Nie udało się pobrać danych", Toast.LENGTH_SHORT).show()
            }
        }
    }
...
interface WasteApi8Service {
    @GET("/__api/mopolskie/wywoz/446")
    suspend fun getWasteSchedule(): Response<WasteResponse>

    companion object {
        private const val BASE_URL = "http://192.168.1.103:3000"

        fun create(): WasteApi8Service {
            return Retrofit.Builder()
                .baseUrl(BASE_URL)
                .addConverterFactory(GsonConverterFactory.create())
                .build()
                .create(WasteApi8Service::class.java)
        }
    }
}
