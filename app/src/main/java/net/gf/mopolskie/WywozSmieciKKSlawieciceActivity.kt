...
class WywozSmieciKKSlawieciceActivity : ComponentActivity() {
    private lateinit var viewModel: Waste7ViewModel

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.waste_kedzierzyn_kozle_slawiecice)
        viewModel = ViewModelProvider(this)[Waste7ViewModel::class.java]
        ...
        lifecycleScope.launch {
            Toast.makeText(this@WywozSmieciKKSlawieciceActivity, "Trwa parsowanie danych...", Toast.LENGTH_SHORT).show()
            val success = viewModel.fetchWaste()
            if (success) {
                showWaste(viewModel.items)
            } else {
                Toast.makeText(this@WywozSmieciKKSlawieciceActivity, "Nie udało się pobrać danych", Toast.LENGTH_SHORT).show()
            }
        }
    }
...
interface WasteApi7Service {
    @GET("/__api/mopolskie/wywoz/445")
    suspend fun getWasteSchedule(): Response<WasteResponse>

    companion object {
        private const val BASE_URL = "https://api.goflux.pl"

        fun create(): WasteApi7Service {
            return Retrofit.Builder()
                .baseUrl(BASE_URL)
                .addConverterFactory(GsonConverterFactory.create())
                .build()
                .create(WasteApi7Service::class.java)
        }
    }
}
