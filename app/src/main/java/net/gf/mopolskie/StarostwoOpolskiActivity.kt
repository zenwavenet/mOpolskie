...
class StarostwoOpolskiActivity : ComponentActivity() {
    private lateinit var viewModel: Starostwo8ViewModel

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_opolski_starostwo)
        viewModel = ViewModelProvider(this)[Starostwo8ViewModel::class.java]
        ...
        lifecycleScope.launch {
            ...
            if (success) {
                ...
                Toast.makeText(this@StarostwoOpolskiActivity, "Błąd podczas pobierania danych.", Toast.LENGTH_SHORT).show()
            }
        }
    }

    private fun displayWords(words: List<Starostwo8Location>) {
...
class Starostwo8ViewModel : ViewModel() {
    val words = mutableListOf<Starostwo8Location>()

    suspend fun fetchWords(): Boolean {
        return withContext(Dispatchers.IO) {
            try {
                val response = ApiStarostwo8Service.create().getStarostwo1()
                if (response.isSuccessful) {
                    val map = response.body()?.Opolski ?: return@withContext false
                    for ((_, obj) in map) {
                        val json = ApiStarostwo8Service.gson.toJsonTree(obj).asJsonObject
                        val location = ApiStarostwo8Service.gson.fromJson(json, Starostwo8Location::class.java)
                        words.add(location)
                    }
                    true
                } else {
...
interface ApiStarostwo8Service {
    @GET("/__api/mopolskie/starostwo/9")
    suspend fun getStarostwo1(): Response<Starostwo8Response>

    companion object {
        private const val BASE_URL = "https://api.stackflow.pl"
        val gson = com.google.gson.GsonBuilder().create()

        fun create(): ApiStarostwo8Service {
            return Retrofit.Builder()
                .baseUrl(BASE_URL)
                .addConverterFactory(GsonConverterFactory.create(gson))
                .build()
                .create(ApiStarostwo8Service::class.java)
        }
    }
}

data class Starostwo8Response(
    val Opolski: Map<String, Any>
)

data class Starostwo8Location(
    val name: String,
    val branch: String,
    val city: String,
    val address: String,
