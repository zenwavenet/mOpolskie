...
class WywozSmieciKKSrodmiescieWWPActivity : ComponentActivity() {
    private lateinit var viewModel: Waste13ViewModel

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.waste_kedzierzyn_kozle_srodmiescie_w_wp)
        viewModel = ViewModelProvider(this)[Waste13ViewModel::class.java]
        ...
        lifecycleScope.launch {
            Toast.makeText(this@WywozSmieciKKSrodmiescieWWPActivity, "Trwa parsowanie danych...", Toast.LENGTH_SHORT).show()
            val success = viewModel.fetchWaste()
            if (success) {
                showWaste(viewModel.items)
            } else {
                Toast.makeText(this@WywozSmieciKKSrodmiescieWWPActivity, "Nie udało się pobrać danych", Toast.LENGTH_SHORT).show()
            }
        }
    }
...
interface WasteApi13Service {
    @GET("/__api/mopolskie/wywoz/451")
    suspend fun getWasteSchedule(): Response<WasteResponse>

    companion object {
        private const val BASE_URL = "http://192.168.1.103:3000"

        fun create(): WasteApi13Service {
            return Retrofit.Builder()
                .baseUrl(BASE_URL)
                .addConverterFactory(GsonConverterFactory.create())
                .build()
                .create(WasteApi13Service::class.java)
        }
    }
}
