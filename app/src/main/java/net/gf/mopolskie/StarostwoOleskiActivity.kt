...
class StarostwoOleskiActivity : ComponentActivity() {
    private lateinit var viewModel: Starostwo7ViewModel

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_oleski_starostwo)
        viewModel = ViewModelProvider(this)[Starostwo7ViewModel::class.java]
        ...
        lifecycleScope.launch {
            ...
            if (success) {
                ...
                Toast.makeText(this@StarostwoOleskiActivity, "Błąd podczas pobierania danych.", Toast.LENGTH_SHORT).show()
            }
        }
    }

    private fun displayWords(words: List<Starostwo7Location>) {
...
class Starostwo7ViewModel : ViewModel() {
    val words = mutableListOf<Starostwo7Location>()

    suspend fun fetchWords(): Boolean {
        return withContext(Dispatchers.IO) {
            try {
                val response = ApiStarostwo7Service.create().getStarostwo1()
                if (response.isSuccessful) {
                    val map = response.body()?.Oleski ?: return@withContext false
                    for ((_, obj) in map) {
                        val json = ApiStarostwo7Service.gson.toJsonTree(obj).asJsonObject
                        val location = ApiStarostwo7Service.gson.fromJson(json, Starostwo7Location::class.java)
                        words.add(location)
                    }
                    true
                } else {
...
interface ApiStarostwo7Service {
    @GET("/__api/mopolskie/starostwo/8")
    suspend fun getStarostwo1(): Response<Starostwo7Response>

    companion object {
        private const val BASE_URL = "https://api.goflux.pl"
        val gson = com.google.gson.GsonBuilder().create()

        fun create(): ApiStarostwo7Service {
            return Retrofit.Builder()
                .baseUrl(BASE_URL)
                .addConverterFactory(GsonConverterFactory.create(gson))
                .build()
                .create(ApiStarostwo7Service::class.java)
        }
    }
}

data class Starostwo7Response(
    val Oleski: Map<String, Any>
)

data class Starostwo7Location(
    val name: String,
    val branch: String,
    val city: String,
    val address: String,
