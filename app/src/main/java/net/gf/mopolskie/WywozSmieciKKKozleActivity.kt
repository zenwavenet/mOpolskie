...
class WywozSmieciKKKozleActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.waste_kedzierzyn_kozle_kozle)
        viewModel = ViewModelProvider(this)[Waste4ViewModel::class.java]
        ...
        lifecycleScope.launch {
            Toast.makeText(this@WywozSmieciKKKozleActivity, "Trwa parsowanie danych...", Toast.LENGTH_SHORT).show()
            val success = viewModel.fetchWaste()
            if (success) {
                showWaste(viewModel.items)
            } else {
                Toast.makeText(this@WywozSmieciKKKozleActivity, "Nie udało się pobrać danych", Toast.LENGTH_SHORT).show()
            }
        }
    }
...
class Waste4ViewModel : ViewModel() {
    ...
    suspend fun fetchWaste(): Boolean = withContext(Dispatchers.IO) {
        try {
            val response = WasteApi4Service.create().getWasteSchedule()
...
interface WasteApi4Service {
    @GET("/__api/mopolskie/wywoz/444")
    ...
    companion object {
        ...
        fun create(): WasteApi4Service {
            return Retrofit.Builder()
                ...
                .build()
                .create(WasteApi4Service::class.java)
        }
    }
}
