...
class WywozSmieciKKKuzniczkaActivity : ComponentActivity() {
    private lateinit var viewModel: WasteViewModel

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.waste_kedzierzyn_kozle_kuzniczka)
        viewModel = ViewModelProvider(this)[WasteViewModel::class.java]
        ...
        lifecycleScope.launch {
            Toast.makeText(this@WywozSmieciKKKuzniczkaActivity, "Trwa parsowanie danych...", Toast.LENGTH_SHORT).show()
            val success = viewModel.fetchWaste()
            if (success) {
                showWaste(viewModel.items)
            } else {
                Toast.makeText(this@WywozSmieciKKKuzniczkaActivity, "Nie udało się pobrać danych", Toast.LENGTH_SHORT).show()
            }
        }
    }
...
class WasteViewModel : ViewModel() {
    ...
    suspend fun fetchWaste(): Boolean = withContext(Dispatchers.IO) {
        try {
            val response = WasteApiService.create().getWasteSchedule()
