...
class WywozSmieciKKPogorzelecActivity : ComponentActivity() {
    private lateinit var viewModel: Waste6ViewModel

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.waste_kedzierzyn_kozle_pogorzelec)
        viewModel = ViewModelProvider(this)[Waste6ViewModel::class.java]
        ...
        lifecycleScope.launch {
            Toast.makeText(this@WywozSmieciKKPogorzelecActivity, "Trwa parsowanie danych...", Toast.LENGTH_SHORT).show()
            val success = viewModel.fetchWaste()
            if (success) {
                showWaste(viewModel.items)
            } else {
                Toast.makeText(this@WywozSmieciKKPogorzelecActivity, "Nie udało się pobrać danych", Toast.LENGTH_SHORT).show()
            }
        }
    }
...
interface WasteApi6Service {
    @GET("/__api/mopolskie/wywoz/442")
    suspend fun getWasteSchedule(): Response<WasteResponse>

    companion object {
        private const val BASE_URL = "https://api.goflux.pl"

        fun create(): WasteApi6Service {
            return Retrofit.Builder()
                .baseUrl(BASE_URL)
                .addConverterFactory(GsonConverterFactory.create())
                .build()
                .create(WasteApi6Service::class.java)
        }
    }
}
